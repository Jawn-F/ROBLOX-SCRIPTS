...
-- NEW UI ELEMENTS FOR MINI-BOSS RESET
local AutoResetOnMiniBossToggle = NormalDungeonBox:AddToggle("AutoResetOnMiniBoss", {
    Text = "Auto Reset on Mini-Boss Defeat",
    Default = config.autoResetOnMiniBoss,
    Tooltip = "Resets dungeon after the mob in the specified room is defeated instead of the final boss.",
    Callback = function(Value)
        autoResetOnMiniBoss = Value
        config.autoResetOnMiniBoss = Value
        saveConfig()
        Library:Notify({Title = "Auto Reset on Mini-Boss", Description = Value and "Enabled" or "Disabled", Time = 2})
    end
})

local MiniBossRoomNumberInput = NormalDungeonBox:AddInput("MiniBossRoomNumber", {
    Text = "Mini-Boss Room Number",
    Default = tostring(config.miniBossRoomNumber),
    Placeholder = "e.g., 6",
    Tooltip = "The room number where the mini-boss is located.",
    Callback = function(Value)
        local num = tonumber(Value)
        if num and num > 0 then
            miniBossRoomNumber = num
            config.miniBossRoomNumber = num
            saveConfig()
        else
            Library:Notify({Title = "Invalid Input", Description = "Please enter a valid number for Mini-Boss Room.", Time = 3})
        end
    end
})
...
ThemeTab:AddButton({
    ...
    Func = function()
        ...
        autoClaimDailyQuest = false
        autoEquipHighestWeapon = false
        autoResetOnMiniBoss = false -- NEW: ensure this loop also stops
...
-- Set values for new UI toggles/inputs
AutoResetOnMiniBossToggle:SetValue(config.autoResetOnMiniBoss)
MiniBossRoomNumberInput:SetValue(tostring(config.miniBossRoomNumber))